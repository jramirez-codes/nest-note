...
func TestSearchHandler(t *testing.T) {
	...
	if err := saveNotebook(mcpDir, notebook{Slug: "greenhouse", Title: "Greenhouse", Summary: "Plants."}); err != nil {
		t.Fatal(err)
	}
	if err := upsertPage(mcpDir, "greenhouse", "Watering Schedule", []string{"water the tomatoes daily"}); err != nil {
		t.Fatal(err)
	}
	if err := saveNotebook(mcpDir, notebook{Slug: "taxes", Title: "Taxes", Summary: "Filing."}); err != nil {
		t.Fatal(err)
	}
	if err := upsertPage(mcpDir, "taxes", "Deadlines", []string{"file by April"}); err != nil {
		t.Fatal(err)
	}
	...
	get := func(url string) *httptest.ResponseRecorder {
		...
		searchHandler(token, root)(rr, req)
		return rr
	}
