...
// fakeCrontab stands in for the crontab binary so deleting a project never
// touches a developer's real one.
type fakeCrontab struct {
	content string
	saves   int
}

func (f *fakeCrontab) io() cronpkg.IO {
	return cronpkg.IO{
...
func TestDeleteProjectHandler(t *testing.T) {
	...
	if cronpkg.HasLine(cron.content, "alpha") {
