// Package update serves /update: pull the repo, rebuild the server binary, and
// restart into it.
package update
...
// Handler runs on the pinned mux. A plain authed POST triggers the
...
func Handler(token, root, repoOverride string) http.HandlerFunc {
