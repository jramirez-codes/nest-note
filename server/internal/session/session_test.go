...
func TestReapDecisionDoneLingers(t *testing.T) {
	...
	if remove, kill := reapDecision(s, now.Add(linger-time.Second)); remove || kill {
	...
	remove, kill := reapDecision(s, now.Add(linger+time.Second))
	if !remove || kill {
...
func TestFindOrCreateAndGet(t *testing.T) {
	reg := &Registry{m: map[string]*Session{}} // no reaper goroutine in test
...
func TestSweepReapsOrphanAndKills(t *testing.T) {
	...
	orphan := &Session{
		id:         "orphan",
		kind:       "code",
		detachedAt: time.Now().Add(-ttlCode - time.Minute),
