...
func newViewManager(bindIP string) *viewManager {
	return &viewManager{bindIP: bindIP, byPort: map[int]int{}}
}
...
func (m *viewManager) start(target int) (int, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// viewStartHandler runs on the pinned mux. The phone posts here (token-authed)
...
func viewStartHandler(token string, enabled bool, m *viewManager) http.HandlerFunc {
