...
export function buildIsLive(status: string | undefined): boolean {
  return status === 'planning' || status === 'building' || status === 'awaiting-validation';
}
 ...
 */
export function cardBuild(card: DashboardCard): { slug: string; status: string; feature: number } | null {
 ...
 */
export async function startBuild(card: DashboardCard, project: string): Promise<BuildInfo> {
  return parseBuild(
    await request(
      '/build/start',
      { card_id: card.id, source: card.source ?? '', project },
  ...
  if (build.features.length === 0) {
    head.push(
      ...
      build.status === 'planning'
        ? '_Claude is writing the project plan…_'
        : '_No plan on disk yet._',
    );
    return head.join('\n');
  }
